function [ sA,sE,sV,sL,sT,dvg ] = intergrate_clean(data,cstes,pars)
% Integrates the mass(/strain) balance equation
%   Detailed explanation goes here

%% Physical parameters
a=pars(1);  % Elasticity
m=pars(2); % Contraction
...
%% Pre-run to equilibrate simulation 
while t<60*data.K0 && dvg==0
  ...
  if dvg==0
        ...
        % Effective depolymerization rate
...
while t<Tend && dvg==0
    ...
    if dvg==0
        ...
        %% Upstream method for the size of filaments (because size as a conserved quantity is *messy*
        Ls(1,Ns)=L0;
